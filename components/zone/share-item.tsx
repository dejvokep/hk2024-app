...
export default async function ShareItem({trans, code, amount, price}: {price: number, trans: Array<{symbol: string, amount: number, price: number}>, code: string, amount: number}) {
    ...
    return <Link href={"/zone/exchange/" + encodeURIComponent(code)}><div className="flex justify-between p-5 w-full tracking-normal text-white bg-black rounded-xl shadow-sm" style={{boxShadow: "inset 0 2px 4px 0 rgba(255, 255, 255, 0.25), 0 -2px 10px 0 rgba(255, 255, 255, 0.1)"}}>
